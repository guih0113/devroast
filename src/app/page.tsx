...
export default async function HomePage() {
  ...
  const dbOffline = stats.dbOffline || previewEntries.dbOffline

  const footerTotal =
    stats.data.total > 0
      ? `${stats.data.total.toLocaleString()} codes roasted`
      : 'be the first to get roasted'
  const footerAvg = stats.data.avgScore ? `avg score: ${stats.data.avgScore}/10` : null
      ...
      <main className="flex flex-col gap-8 px-10 pt-20">
        ...
        <div className="mx-auto flex w-full max-w-4xl flex-col gap-6">
          ...
          <div className="flex items-center justify-center py-2">
            <span className="font-mono text-xs text-zinc-600" suppressHydrationWarning>
              {previewEntries.data.length > 0
                ? `// showing ${previewEntries.data.length} of ${stats.data.total.toLocaleString()} · visit full leaderboard ↓`
